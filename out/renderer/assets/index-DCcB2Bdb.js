...
const useUIStore = create((set, get) => ({
  ...
  toggleProject: (id2) => {
    ...
    set({ expandedProjectId: current === id2 ? null : id2 });
  },
  setCompletingStamp: (id2) => set({ completingStampId: id2 }),
...
const useProjectStore = create((set, get) => ({
  ...
  updateProject: async (id2, updates) => {
    await window.api.projects.update(id2, updates);
    await get().fetchProjects();
  },
  deleteProject: async (id2) => {
    const { [id2]: _, ...rest } = get().projectMap;
    set({ projectMap: rest });
    await window.api.projects.delete(id2);
  },
  archiveProject: async (id2) => {
    await window.api.projects.archive(id2);
    const { [id2]: _, ...rest } = get().projectMap;
    set({ projectMap: rest });
  },
  ...
  removeStampFromProject: async (id2, projectId) => {
    await window.api.projectStamps.remove(id2);
    await get().fetchProjectStamps(projectId);
    await get().fetchProjects();
  },
  ...
  setStampStatus: async (id2, projectId, status) => {
    await window.api.projectStamps.setStatus(id2, status);
    await get().fetchProjectStamps(projectId);
    await get().fetchProjects();
  }
}));
function ProgressBar({ total, completed, className = "" }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex items-center gap-2 ${className}`, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-xs text-panel-text-muted tabular-nums w-8 text-right", children: [
      percent,
      "%"
    ] })
  ] });
...
function StampBadge({ name, color, size = "sm", className }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Badge, { color, size, className, children: name });
}
...
function StampItem({ item, projectId }) {
  const setCompletingStamp = useUIStore((s) => s.setCompletingStamp);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      className: `flex items-center gap-2 px-2 py-1.5 rounded-md group ${item.status === "completed" ? "opacity-60" : "hover:bg-panel-hover"}`,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `text-xs ${statusStyle[item.status]}`, children: statusIndicator[item.status] }),
...
function CommitDialog({ projectStampId, projectId, stampName }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: true, title: `完成：${stampName}`, onClose: () => setCompletingStamp(null), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2 justify-end", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "ghost", size: "sm", onClick: () => setCompletingStamp(null), children: "取消" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { size: "sm", onClick: handleSubmit, disabled: saving, children: saving ? "提交中..." : "确认完成" })
    ] })
  ] }) });
}
function ProjectDetail({ projectId }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "animate-slide-down", children: [
    stamps.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-panel-text-muted py-2 px-2", children: "暂无便签" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-0.5", children: stamps.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsx(StampItem, { item, projectId }, item.id)) }),
...
function ProjectCard({ project }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-panel-border bg-panel-surface overflow-hidden", children: [
    ...
    isExpanded && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "border-t border-panel-border px-2 py-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ProjectDetail, { projectId: project.id }) })
  ] });
}
...
const useStampStore = create((set, get) => ({
  ...
  updateStamp: async (id2, updates) => {
    const prev = get().stampMap[id2];
    if (!prev) return;
    set((state) => ({
      stampMap: { ...state.stampMap, [id2]: { ...prev, ...updates } }
    }));
    try {
      const updated = await window.api.stamps.update(id2, updates);
      set((state) => ({ stampMap: { ...state.stampMap, [id2]: updated } }));
    } catch {
      set((state) => ({ stampMap: { ...state.stampMap, [id2]: prev } }));
    }
  },
  deleteStamp: async (id2) => {
    const prev = get().stampMap[id2];
    const { [id2]: _, ...rest } = get().stampMap;
    set({ stampMap: rest });
    try {
      await window.api.stamps.delete(id2);
    } catch {
      if (prev) set((state) => ({ stampMap: { ...state.stampMap, [id2]: prev } }));
    }
  }
}));
function StampPicker({ selected, onChange }) {
  ...
  const toggle = (id2) => {
    if (selected.includes(id2)) {
      ...
      onChange([...selected, id2]);
    }
  };
  if (stamps.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-panel-text-muted py-2", children: "请先创建便签" });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-1 max-h-32 overflow-y-auto", children: stamps.map((stamp) => {
    const isSelected = selected.includes(stamp.id);
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        type: "button",
        className: `w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-left transition-colors ${isSelected ? "bg-panel-accent/20 border border-panel-accent/40" : "hover:bg-panel-hover border border-transparent"}`,
        onClick: () => toggle(stamp.id),
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `text-xs ${isSelected ? "text-panel-accent" : "text-panel-text-muted"}`, children: isSelected ? "✓" : "○" }),
...
function CreateProjectForm() {
  ...
  const [selectedStamps, setSelectedStamps] = reactExports.useState([]);
  const [saving, setSaving] = reactExports.useState(false);
      ...
      "div",
      {
        ...
        children: [
          ...
          deletingId === stamp.id ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { size: "sm", variant: "danger", onClick: () => handleDelete(stamp.id), children: "确认" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { size: "sm", variant: "ghost", onClick: () => setDeletingId(null), children: "取消" })
          ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
...
function CreateStampForm() {
  const [name, setName] = reactExports.useState("");
  const [color, setColor] = reactExports.useState(DEFAULT_STAMP_COLORS[0]);
  const [saving, setSaving] = reactExports.useState(false);
...
function ViewContent() {
  ...
  switch (view) {
    case "projects":
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ProjectList, {});
    case "create-project":
      return /* @__PURE__ */ jsxRuntimeExports.jsx(CreateProjectForm, {});
    case "stamps":
      return /* @__PURE__ */ jsxRuntimeExports.jsx(StampLibrary, {});
    case "create-stamp":
      return /* @__PURE__ */ jsxRuntimeExports.jsx(CreateStampForm, {});
    default:
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ProjectList, {});
  }
}
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(PanelShell, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(ViewContent, {}) });
}
